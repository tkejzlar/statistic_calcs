...
# rspec ./spec/lib/statistic_calcs/hypothesis_test/gumbel_maximum_goodness_and_fit_spec.rb
...
RSpec.describe StatisticCalcs::HypothesisTest::GoodnessAndFit do
  ...
  context 'Happy path' do
    ...
    let!(:data) { StatisticCalcs::ChiSquareContrast::GumbelMaximumGoodnessAndFit.new(options) }
    describe 'Chi square contrast test' do
      it 'should fill all the attributes' do
        expect(subject.null_hypothesis).to eq('Xc^2 <= X^2(1 - alpha, V). 102.4852 <= 9.48773')
        expect(subject.alternative_hypothesis).to eq('Xc^2 > X^2')
        expect(subject.critical_fractil).to eq(102.4852)
        expect(subject.reject).to eq(true)
        expect(subject.reject_condition).to eq('Xc^2 > X^2 -> reject H0. `102.49 > 9.49` -> true')
      end
    end
  end
